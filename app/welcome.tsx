...
import { Screen, Button, Card, Chip, SectionHeader } from '@/components/ui';
...
export default function WelcomeScreen() {
  ...
  const nextStep = () => setStep(s => Math.min(2, s + 1) as Step);
    ...
    <View style={S.dots} accessibilityLabel={`Step ${step + 1} of 3`}>
      {[0, 1, 2].map(i => (
        <View
          ...
          style={[
            S.dot,
            { backgroundColor: i === step ? theme.tint : theme.cardAlt },
            i === step && S.dotActive,
          ]}
        />
      ))}
  ...
  const footer = (
    <View style={S.footer}>
      {dots}
      {step === 0 && <Button label="Get started" icon="arrow-forward" iconRight onPress={nextStep} />}
      {step === 1 && <Button label="Continue" icon="arrow-forward" iconRight onPress={nextStep} />}
      {step === 2 && (
        <>
  ...
  return (
    <Screen
      onBack={step === 0 ? false : () => setStep(s => (s - 1) as Step)}
      contentStyle={S.content}
      footer={footer}
    >
      {step === 0 && (
        <Animated.View key="s0" entering={FadeIn.duration(220)} exiting={FadeOut.duration(120)}>
      ...
      {step === 2 && (
        <Animated.View key="s2" entering={FadeIn.duration(220)} exiting={FadeOut.duration(120)}>
